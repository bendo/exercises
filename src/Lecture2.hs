...
removeAt y xs
    | y < 0       = (Nothing, xs)
    | null second = (Nothing, xs)
    | otherwise   = (Just (head second), first ++ drop 1 second)
    where
        first  = take y xs
        second = drop y xs
...
-}
-- not sure how to check if list is infinite
dropSpaces :: String -> String
dropSpaces = filter ( not . isSpace ) . take 100
...
mergeSort [x,y]
    | x < y = [x,y]
    | otherwise = [y,x]
mergeSort xs = merge (mergeSort (fst lists)) (mergeSort (snd lists))
    where
        lists = splitAt (div (length xs) 2) xs
